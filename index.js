...
//display characters under age 25
renderByAge(users, 25, "age-filter-list");
...
function renderWithErrors(array, elementId, errorElementId) {
  ...
  const valid = array.filter((u, index) => {
    if (!u.name) {
      ...
      console.error(msg);
      errors += `<div class="error-message">${msg}</div>`;
      return false;
    }
    return true;
  });
...
// broken test data for exercise 6 is generated by AI
const brokenUsers = [
